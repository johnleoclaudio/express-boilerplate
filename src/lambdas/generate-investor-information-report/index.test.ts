...
const { generateInvestorInformationReport } = handler;
...
describe('generateInvestorInformationReport', () => {
  beforeEach(async () => {
    await umzug.up();
  });
  ...
  it.only('works without specific ownerIds', async () => {
    // container
    ...
    const res = await generateInvestorInformationReport({
      ...event,
    });
