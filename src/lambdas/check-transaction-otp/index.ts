...
exports.checkTransactionOtp = async (event: any) => {
  let response;
  ...
  try {
    ...
    const checkTransactionOtp = container.get(
      Types.CheckTransactionOtpBondUser,
    );
    ...
    const otpRes = await checkTransactionOtp.execute(
      {
        accessToken,
        otp,
        transactionType,
