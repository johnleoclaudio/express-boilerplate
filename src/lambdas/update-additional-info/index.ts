...
exports.updateAdditionalInfo = async (event: any) => {
  let response;
  ...
  try {
    ...
    const updateAdditionalInfo = container.get(Types.UpdateAdditionalInfo);

    const data = await updateAdditionalInfo.execute({
      ...params,
      accessToken: access_token,
      type,
      scope,
