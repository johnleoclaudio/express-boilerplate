...
exports.updateClientInfo = async (event: any) => {
  let response;
  ...
  try {
    ...
    const updateClientInfoFeature = container.get(Types.UpdateClientInfo);

    const updateClientInfoResponse = await updateClientInfoFeature.execute({
      ...params,
      accessToken: access_token,
    });

    // filter out sensitive client information
    const data = updateClientInfoResponse.dataValues;
