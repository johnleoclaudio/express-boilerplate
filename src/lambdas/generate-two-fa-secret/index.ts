...
exports.generateTwoFaSecret = async (event: any) => {
  let response;
  try {
    const { username, password } = JSON.parse(event.body);

    const generateTwoFaSecret = container.get(Types.GenerateTwoFaSecret);

    const result = await generateTwoFaSecret.execute({
      username,
      password,
    });
