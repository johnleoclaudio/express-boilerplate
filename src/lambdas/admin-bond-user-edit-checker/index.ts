...
exports.adminEditBondUserRequestChecker = async (event: any) => {
  let response;
  ...
  try {
    ...
    const editBondUserInfoChecker = container.get(
      Types.BondUserInfoEditRequestChecker,
    );

    const data = await editBondUserInfoChecker.execute(
      {
        ...params,
        adminId: admin.owner.id,
        checkerDetails: {
          id: admin.owner.id,
          email: admin.owner.username,
        },
      },
      dbTxn,
    );
