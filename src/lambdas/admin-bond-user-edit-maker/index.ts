...
exports.adminEditBondUserRequestMaker = async (event: any) => {
  let response;
  ...
  try {
    ...
    const editBondUserInfo = container.get(Types.BondUserInfoEditRequestMaker);

    const data = await editBondUserInfo.execute(
      {
        ...params,
        editedBy: admin.owner.id,
        makerDetails: {
          id: admin.owner.id,
