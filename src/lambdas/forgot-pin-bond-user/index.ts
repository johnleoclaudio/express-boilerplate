...
exports.forgotPinBondUser = async (event: any) => {
  let response;
  const logger = container.get(Types.LoggerService);
  const loggerParams = {
    layer: 'lambda',
    name: 'forgotPinBondUser',
    type: 'request',
    message: event,
  };
  ...
  try {
    const { deviceId, deviceName, identifier } = JSON.parse(event.body);

    const forgotPinFeature = container.get(Types.ForgotPin);

    const data = await forgotPinFeature.execute(
      {
        deviceId,
        deviceName,
        identifier,
